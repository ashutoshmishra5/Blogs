...
const BlogDisplayComponent = ({ id }) => {
    ...
    return (
        <div>
            ...
            <div className='text-xl py-4 px-4 bg-slate-200 font-semibold'>{blog.title}</div>
            ...
            <div className="flex justify-center py-4 px-4 bg-slate-200">
                <Image className="object-cover" src={blog.imgUrl} height={1000} width={1000} alt=""/>
            </div>

            <div className='text-md py-4 px-4 bg-slate-200 text-justify'>{paragraphs.map((paragraph,index)=>(<p key = {index} className={paragraph.length<40?'font-semibold my-4':''}>{paragraph}</p>))}</div>
